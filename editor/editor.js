...
ÉTAPE 2 — Créer la page WordPress
  ...
  2. Titre "eBook ${slug}" + slug clair (ex: ebook-${slug}).
...
async function findPageId(siteUrl, slug, authHeader) {
  // On ne s'appuie pas sur le paramètre "search" de l'API (recherche floue
  // WordPress, peu fiable selon les sites) : on récupère toutes les pages et
  // on filtre nous-mêmes par modèle + slug/titre, pour éviter de créer un
  // doublon quand une page existante n'est pas retrouvée.
  const targetSlug = slug.toLowerCase();
  let allPages = [];
  let page = 1;
  const maxPages = 10; // jusqu'à 1000 pages, largement suffisant

  while (page <= maxPages) {
    const res = await fetch(
      `${siteUrl}/wp-json/wp/v2/pages?per_page=100&page=${page}&status=publish,future,draft,pending,private&context=edit`,
      { headers: { Authorization: authHeader } }
    );
    if (!res.ok) {
      if (page > 1) break; // on a dépassé la dernière page
      let json = null;
      try { json = await res.json(); } catch (e) { /* pas de corps JSON */ }
  ...
  const match = allPages.find(p => {
    ...
    return tpl.includes('page-landing-guide.php') &&
      (p.slug.toLowerCase().includes(targetSlug) || (p.title?.raw || p.title?.rendered || '').toLowerCase().includes(targetSlug));
  });
  if (match) return match.id;
  // repli : première page utilisant le bon modèle, même sans correspondance de slug
  const fallback = allPages.find(p => (p.template || '').includes('page-landing-guide.php'));
  return fallback ? fallback.id : null;
}

async function createPage(siteUrl, slug, authHeader) {
  const title = `eBook ${slug.charAt(0).toUpperCase()}${slug.slice(1)}`;
...
document.getElementById('publish-btn').onclick = async () => {
  ...
  try {
    ...
    const pagePayload = { acf: acfPayload };
    if (acfPayload.hero_cover_image) {
      // Utilise la cover comme photo à la une (illustration principale de la
      // page WordPress, utile pour le partage sur les réseaux et le SEO).
      pagePayload.featured_media = acfPayload.hero_cover_image;
    }
